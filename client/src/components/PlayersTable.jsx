...
export default function PlayersTable({ playerId, position }) {
  const classes = useStyles();
  const [tablePlayers, setTablePlayers] = useState(new Map());

  if (
    ['Portero', 'Defensa', 'Mediocentro', 'Delantero'].includes(
      position,
    )
  ) {
    const players = <PlayersByPosition position={position} />;
    // players.forEach((p,i) => setTablePlayers(tablePlayers.set(i, p)))
  } else if (!tablePlayers.has(playerId)) {
    setTablePlayers(
      tablePlayers.set(playerId, <Player playerId={playerId} />),
    );
  }
  useEffect(() => {
  ...
  return (
    <Paper className={classes.root}>
      <Table className={classes.table}>
        ...
        <TableBody>{Array.from(tablePlayers.values())}</TableBody>
      </Table>
    </Paper>
  );
}
